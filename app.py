...
if "user_db" not in st.session_state:
    st.session_state.user_db = {"spade": "kashspade123", "kevin": "tongkrongan70b"}
...
# --- 5. INTERFACE: HIGH-END AUTH PAGE ---
def show_auth_page():
    st.write("\n\n")
    st.markdown('''
        <div class="auth-card">
        ...
        if submit:
            if "Sign In" in mode:
                if u_input in st.session_state.user_db and st.session_state.user_db[u_input] == p_input:
                    ...
                    st.rerun()
                else: st.error("Akun salah atau belum terdaftar, bre.")
            else:
                if len(u_input) < 3 or len(p_input) < 5: st.error("Username min. 3 huruf, Password min. 5 huruf!")
                elif u_input in st.session_state.user_db: st.error("Username sudah terpakai!")
                else:
                    st.session_state.user_db[u_input] = p_input
                    st.success("Registrasi sukses! Silakan pindah ke 'Sign In'.")
